...
import { useActiveMatchesForMe, useOpenMatches, useJoinOpenMatch, useChallenge, type GameKind } from '@/lib/games';
...
export function GameHub() {
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 space-y-6">
          ...
          {PHASE1_KINDS.filter((k) => GAME_REGISTRY[k]?.mode === 'solo').map((k) => {
            ...
            return (
              <Link key={k} to={`/spiele/solo/${k}`}
                ...
                <div className="text-xs text-forest-400 mt-1">{meta.short}</div>
              </Link>
            );
          })}
        </div>
      ...
      {/* Live PvP + Async — Neues Spiel starten */}
      <section>
      ...
      {(open.data?.length ?? 0) > 0 && (
        <section>
          ...
          <ul className="space-y-2">
            {open.data!.map((o) => {
              if (o.challenger_id === myId) return null; // eigene Lobby nicht zum Joinen
              return (
                <li key={o.match_id}
...
function PvPLauncher({ kind, label, emoji, short, myId }:
  ...
  async function startOpenLobby() {
    // games_create_match ohne opponent → pending Lobby
