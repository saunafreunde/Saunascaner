// Vercel Cron — kombinierter Reminder
// 1) WM-Tipp-Reminder: für Mitglieder mit Push-Sub die Spiele in den nächsten 12h nicht getippt haben
// 2) Bewertungs-Fenster-Reminder: für anwesende Mitglieder die Aufgüsse noch nicht bewertet haben
// Idempotent durch Tag-basiertes Deduplizieren im Browser (Notification-API merged gleichen tag).
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Pro Member nur die FRÜHESTE Match-Reminder (sonst spammt es)
  ...
  for (const r of (ratingList ?? [])) {
    ...
    if (!existing || new Date(r.end_time) > new Date(existing.end_time)) {
      ratingByMember.set(r.member_id, r);
    }
  }

  // ── Subscriptions vorab laden (eine Query) ──
  ...
  for (const [memberId, r] of ratingByMember.entries()) {
    ...
    if (minsLeft < 5) continue; // zu kurz, nicht mehr lohnenswert
