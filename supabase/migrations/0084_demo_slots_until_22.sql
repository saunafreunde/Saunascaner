-- 0084_demo_slots_until_22.sql
-- ─────────────────────────────────────────────────────────────────────────
-- DEMO-MODUS für Präsentation: Aufguss-Slots erweitert auf bis 22:00 Uhr
-- (vorher 20:00). Garantie-Rhythmus läuft alternierend weiter.
--
-- Zurück-Migration: einfach garantie_temperature_for() aus Migration 0083
-- wieder einspielen (Maximalstunde dort = 20).
--
-- Synchron mit TS-Helper in src/lib/garantie.ts (LAST_SLOT_HOUR_DEMO = 22)
-- ─────────────────────────────────────────────────────────────────────────
...
declare
  ...
  v_hour  int;
  v_start_hour int;
  v_last_hour  int := 22;  -- DEMO: vorher 20
...
comment on function public.garantie_temperature_for(timestamptz) is
  'DEMO: Garantie-Slot-Temperatur (80/100°C) bis 22 Uhr (statt 20). Mo nur wenn schedule_settings.monday_open=true.';
